...
- 1차 목표:
  - 먼저 `85%`까지 끌어올린다.
- 최종 목표:
  - `사용자가 실제로 인정하는 자리`와 `모델이 먼저 고른 자리`를 `95% 이상` 일치시킨다.
- 현재 가장 부족한 것:
  - `가장 편한 하루` 압축
  - `결과 안 좋은 자리` 자동 감점
  - `매도 자리` 규칙화
...
- `이평선에 닿는다`는 것은 단순 접촉이 아니라 `매수/매도 근거가 생긴다`는 뜻이다.
- 이평선이 아래에서 받쳐주면 편하고, 위에 있으면 `머리 맞고 내려오는 느낌`이라 부담스럽다.
- `급등주`일수록 `5/20일선` 같은 단기 이평이 중요하다.
- `천천히 오르는 종목`일수록 `60/120/240/480` 장기 이평 중요도가 올라간다.
- `240일선`은 생각보다 중요하고, 경우에 따라 `480일선`도 본다.
- `빗각 미돌파`는 강한 감점이지만 직접 측정은 어렵다.
- `빗각 돌파`는 상승 가능성을 높이지만, 동시에 급등/급락 변동성도 커진다.
- `좋은 자리`, `사고 싶은 자리`, `무서운 자리`, `늦은 자리`를 분리해야 한다.
- `좋은 차트`와 `실제 매수할 자리`는 다르다.
- `도전 가능 자리`와 `추천 가능한 자리`도 다르다.
- `결과 안 좋은 자리`는 추천 우선순위에서 깎아야 한다.
- `안 샀다`는 사실만으로 차트 감점을 주면 안 된다.
  - `종목이 너무 많음`, `포트 비중`, `다른 종목 우선` 같은 사유를 분리해야 한다.
- `장기 우하향`, `신저가 진행형`, `상승 구조 부재`는 강한 제외다.
- `주봉/월봉`은 무조건 배제용이 아니라 `확률 가중치`로 쓴다.
- `월봉/주봉이 받쳐줘도 일봉 지지라인이 없으면` 실전 매수는 애매할 수 있다.
...
- 웹 상단에 `총점` 추가
- 하단에 `시장/지수 강도`, `펀더멘털/배당 지표` 표시
- `저장된 과거 사례`는 맨 아래로 이동
- KR 펀더멘털은 `pykrx -> 네이버 금융 크롤링 -> 패스` 순서로 fallback
- 기준일 입력 기본값은 `오늘 날짜`
- `직접 입력 참고 지표` 제거
- `급등 전조 점수(breakout_setup_score)` 추가
- 코멘트를 더 디테일하게 수정
  - 이평이 지지인지 저항인지
  - 편한 자리인지
  - 급등 전조 흔적인지
  - 120/240선 심리 포인트가 있는지
...
- 이미 `_cache/chart_case_notes.json` 등에 저장된 날짜를 `새로 분석한 후보`처럼 다시 제시하면 안 된다.
- 최근에는 내가 여러 종목에 비슷한 날짜를 반복 제시하는 문제가 있었으므로, 다음 세션부터는 `종목 고유 파동` 위주로 날짜를 뽑아야 한다.
- 사용자는 `결과 안 좋은 자리 추천`을 매우 싫어한다.
- `좋은 자리`여도 `손절 많이 나는 종목`은 별도 감점해야 한다.
- 다음 단계에서 꼭 해야 할 일:
  - `급등 전조 점수`를 카카오, 에코프로, 위메이드류 사례와 계속 대조
  - `매도 자리` 규칙화 시작
  - `결과 안 좋음`을 점수상 더 강하게 반영
...
이 프로젝트 목표는
내가 실제로 인정하는 차트 매수자리와 네가 먼저 고른 자리가 95% 이상 일치하도록 보정하는 것이다.

기준:
- 시장보다 차트 자리가 우선
- 빗각, 지지저항, 20/60/120/240/480 이평, 주봉, 월봉, 거래량, RSI, 이격도, 박스권을 같이 본다
- 빗각은 직접 수치화하지 않고 다른 지표 흔적으로 근사한다
- 좋은 자리 / 사고 싶은 자리 / 무서운 자리 / 늦은 자리를 구분한다
- 추격매수는 메인 시스템에 넣지 않는다
- 점수는 일봉 중심, 주봉/월봉은 약한 보정이지만 확률을 높이는 쪽으로 본다

작업 방식:
- 네가 종목 하나의 과거 좋은 자리 후보 날짜를 먼저 제시
- 내가 정답/근접/제외와 결과 좋음/나쁨/손절 가능으로 보정
- 상승 후 눌림 vs 횡보 중, 빗각 미돌파 감점, 240/480일선, 주봉/월봉 반영을 계속 강화
- 이미 저장된 케이스 날짜를 새로 분석한 것처럼 재사용하지 말 것
- 종목 하나 끝날 때마다 진행도를 같이 적을 것

추가 목표:
- 기존 매수 자리 점수와 별도로 `급등 전조 점수`를 계속 검증한다
- 매칭률이 높고 결과가 좋으면 채택, 아니면 폐기한다
- 다음 단계는 `매도 자리` 규칙화다

지금 상태:
- 기존 매수 자리 점수 정규화: 84~85%
- 날짜 매칭률: 81%
- 급등 전조 점수: 51~54%
- 전체 진행도: 80%
